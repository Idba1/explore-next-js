...
const specialblog = () => {
    return (
        <div>
            {
                indoorPlants.map((indoorPlant) => <div className='border-2 p-2' key={indoorPlant.tree}>
                    <h1>{indoorPlant.title}</h1>
                    <h3>{indoorPlant.description}</h3>
                    <button className='bg-[#FFD700] text-white px-5 py-2 rounded-sm hover:text-red-500 hover:font-bold cursor-pointer'>
                        <Link href={`/blog/specialblog/${indoorPlant.tree}`}>Details</Link>
                    </button>
                </div>)
            }
        </div>
